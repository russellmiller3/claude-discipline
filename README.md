...
| `pod-cost-circuit-breaker` | PostToolUse(Bash) | A mid-turn dumb timer: halts a paid pod that's been up past a threshold with no fresh *job*-liveness probe (pod alive ≠ job alive — a status-only monitor bled real money for hours). Escape: `POD_COST_BREAKER_OK` |
