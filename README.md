...
| `ceremony-rabbithole-guard` | Stop | Catches the session slipping into "ceremony" — a streak of ≥4 infra-only commits (tooling/config/docs) with no commit landing real product value, or the same external op retried ≥3×. Nudges: bank what works, advance the deliverable or hand off. Project-agnostic. Override `ceremony-ok` |
