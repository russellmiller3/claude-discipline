...
| `repo-docs-current` | SessionStart | **Every repo gets ONE trustworthy entry document.** Reports, once per repo per session: no `TRUTH.md`; a document map that no longer matches the files on disk; a `HANDOFF.md` over 20 KB (it became a diary, not a queue); or a work log more than 4 commits behind. Warns rather than blocks — adoption, not obstruction. Backed by `scripts/repo-truth-doc.mjs`, which regenerates the map, the branch table and the recent-work list from the filesystem and git, rewriting **only between its markers** so hand-written judgement survives; a file with no markers is left alone, so a repo with its own generator is never fought over. |
| `found-it-fix-it-guard` | Stop | **You find it, you fix it.** Blocks a reply that names a real defect and then disowns it ("a separate concern", "out of scope for this", "pre-existing, not mine", "leaving it for the next session"). Silent when the same reply shows the defect being fixed, and when the deferral is genuinely destructive, needs the human's decision, or exceeds the budget ceiling. |
...
| `pod-full-run-capacity-guard` | PreToolUse(Bash) | Blocks a full-training paid GPU launch unless a full-*shape* capacity smoke ran this session — a one-example wiring smoke under-measures VRAM and the full run OOMs. Escape: `CAPACITY_SMOKE_OK: <evidence>` |
| `pod-teardown-rescue-guard` | PreToolUse(Bash) | Blocks a raw pod DELETE (`DELETE .../pods/`, `runpodctl remove pod`, `--delete-pod`) unless results were rescued first — a pod delete is irreversible. Escape: `KILL_WITHOUT_RESCUE_OK: <why>` |
| `pod-cost-circuit-breaker` | PostToolUse(Bash) | A mid-turn timer: halts a paid pod with no fresh job-liveness proof; direct probes and current RunPod Monitor `ALIVE` events refresh it, while status-only polls do not. Escape: `POD_COST_BREAKER_OK` |
| `main-task-focus-guard` | Stop+UserPromptSubmit | Blocks drift from the recorded MAIN TASK into unauthorized side quests on autonomous turns (records `MAIN TASK:` markers). Escape: `SIDEQUEST_OK: <why>` |
...
| `bluf-first-line-guard` | Stop | Blocks stop when you asked a direct question and the reply's first line isn't the answer — a heading, a table row, "Let me check…", or a 30-word sentence with the verdict buried inside it. Length guards cap how MUCH gets written; this one checks that the answer comes FIRST. Skips the compass line so the two can't fight |
...
| `ceremony-rabbithole-guard` | Stop | Catches the session slipping into "ceremony" — a streak of ≥4 infra-only commits (tooling/config/docs) with no commit landing real product value, the same external op retried ≥3×, or the same whole-project test/lint/build gate proving success twice against unchanged code. Nudges: bank what works, advance the deliverable or hand off. Project-agnostic. Override `ceremony-ok` / `verification-rerun-ok` |
| `pod-crash-diagnosable-guard` | Stop + PreToolUse(Write/Edit) | A paid-cloud-job crash must be diagnosed from a real traceback, not guessed: blocks a worker "fix" after a crash signal when no traceback was read this session, and blocks a crash-branch delete that has no artifact copyback before it. Override `pod-blind-fix-ok` |
| `no-core-delegation-guard` | PreToolUse(Agent) | Agents OFF by default: DENIES every Agent spawn without a recorded per-session signoff (env/sentinel; prompt text cannot self-approve), and — even with signoff — BLOCKS delegating an EDIT to load-bearing code (per-repo `.claude/core-paths.txt`). Reading/auditing core and editing peripheral code pass. Override: `CORE_AGENT_OK`. For teams that want subagents on by default, toggle it off |
| `heredoc-file-content-guard` | PreToolUse(Bash) | Steers file-content creation to the Write tool: blocks a Bash heredoc redirected to a file when the body looks like code/markup (quote-heavy bodies mis-parse and fail the whole command). Program-stdin heredocs (`git commit -F-`, `python <<PY`) and one-line config heredocs pass. Override `HEREDOC_OK` |
| `destructive-safe-by-default-guard` | PreToolUse(Write/Edit) | A script that can do an irreversible destroy (delete/terminate/rm -rf/DROP/force-push/…) must default to list/dry-run: blocks new code that destroys unconditionally with no explicit confirm gate (`--confirm`/`--yes`/`--force`/`CONFIRM`). So a "check" run can never delete. Override `destructive-default-ok` |
...
| `destructive-on-loose-error-guard` | PreToolUse(Write/Edit) | Blocks code that gates an irreversible destroy (delete/terminate/kill/rm -rf) on a loose substring test of an error message — a fuzzy `"x" in str(error)` matches the wrong error and destroys on a benign/"wait"/alive case. Branch on the exact failure token. Override `LOOSE_ERROR_DESTROY_OK` |
| `root-cause-first` | PostToolUse | After repeated same-symptom fixes, forces a state-probe before the next swing |
| `local-server-lifecycle` | PreToolUse(Write/Edit/MultiEdit) | Requires shutdown, stale-owner, and visible-app-close handling on real local server launchers; plain shell jobs and `--start-*` arguments are not server launches |
...
| `discipline-sync` | Stop | The sibling of `hookbook-sync` for this published kit. A hook change must complete the full publish loop before the turn ends: the kit copy matches the live `~/.claude/hooks/` source, the kit's **README/docs are updated**, both repos are committed, and the kit is **pushed to GitHub** — the hook auto-pushes it (kit repo only) and only blocks if commits remain unpushed. So a shipped guard never drifts from the version that runs, and "published" means actually on GitHub, not just committed locally. A write to a hook file that a PreToolUse guard DENIED never counts as an edit — the publish loop keys on each tool call's `is_error` result, so a blocked/rejected write (the file never landed) can't turn the loop on for a phantom hook. Override: `DISCIPLINE_NO_PUSH=1` (gate without push) or `DISCIPLINE_SYNC_OVERRIDE=1` |
...
| `ross-perot-guard` | Stop | "Lead, don't ask." Blocks every assistant question or hand-off unless it protects a concrete destructive action or an explicit estimate above the $5 budget gate. No survey, design, browser, preference, or model-authored override escape. |
