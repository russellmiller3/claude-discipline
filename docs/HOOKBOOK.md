...
### `compass-line-guard` · Stop
Blocks stop when a WORKING turn (any Edit/Write/NotebookEdit/Bash/PowerShell/Agent/Workflow tool call this turn) ends in a reply that doesn't OPEN with a plain-English compass line — "🧭 Mission: \<overall goal> · this step: \<what just happened> · why: \<what it unlocks>". Advisory reminders of this same standard get compressed out of context under pressure, so this hook enforces the outcome instead of suggesting it. A pure chat/explaining turn (only Read/Grep/Glob/ToolSearch) is exempt. A first line of `## 🚀 TL;DR` (a big-picture-narration skill's own header) also satisfies the rule by construction. **Anti-loop rail:** reuses the same `stop_hook_active` convention as `never-idle`/`no-backcompat` — blocks the first pass, always allows the re-entrant pass caused by its own prior block. Locked by `compass-line-guard.test.mjs` (16 checks).
...
### `unsafe-main-ref-write-guard` · PreToolUse(Bash|PowerShell)
Blocks the exact pattern that caused three separate silent history-loss incidents in one background-agent session: a two-arg `git update-ref refs/heads/main <sha>` (or `git branch -f main`/`git checkout -B main <sha>`), which skips git's fast-forward safety check entirely. A `git worktree` isolates the working tree, NOT `refs/heads/main` — that ref is shared across every worktree of a repo, so an agent landing this way can silently overwrite a sibling agent's just-landed commit. Points agents at `scripts/safe-merge-to-main.sh` — a real mkdir-lock mutex around rebase→test→land, landing via a three-arg compare-and-swap `update-ref <new> <old>` (explicitly allowed here — only the unconditional two-arg form is blocked). **Override:** `SAFE_MERGE_OVERRIDE=1`. Locked by `unsafe-main-ref-write-guard.test.mjs` (14 tests).
