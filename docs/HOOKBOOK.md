...
Blocks `git commit` while the TARGET repo's branch is `main`/`master`. Resolves the repo the commit actually runs in — a leading `cd <path> &&` or `git -C <path>` is honored, so committing to another repo's fix branch is not judged by the session repo's branch (that false-block fixed 2026-07-01). Work on a branch, merge when done. Fails open if not a git repo. **Override:** `COMMIT_MAIN_OVERRIDE=1`. Locked by `no-commit-to-main.test.mjs` (9 tests).
