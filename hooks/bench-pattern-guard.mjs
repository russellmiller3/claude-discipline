#!/usr/bin/env node
// =============================================================================
// bench-pattern-guard — every benchmark RUNNER must be parallel + progress-
//                       emitting + durable & idempotent, or the write is BLOCKED.
// =============================================================================
//
// The rule: any benchmark / eval / sweep should run its tasks in PARALLEL, emit
// regular progress as it goes, and be DURABLE + IDEMPOTENT. A serial for-loop that
// goes silent for 20 minutes and can't resume is the failure mode this prevents.
//
// THE PATTERN (all three, non-negotiable for a bench/eval/sweep RUNNER):
...
//                    failures early.
//   2. PROGRESS    — emit plain-English progress as each task advances (a pulse /
//                    progress log / event), so a long run is observable live instead
...
const ITERATES_TASKS = /\b(TASKS|scenarios|taskQueue|testCases|probes|cases)\b/;
// Generic "this drives a model/agent" signals — common SDK/HTTP entry points plus the conventional
// per-task runner names. No project-specific brain factory is hardcoded.
const CALLS_AGENT = /(runAgent|callModel|callChatCompletion|chat\.completions|api\.anthropic|\/v1\/messages|openai|openrouter|generateContent|invokeModel|runOneTask|runScenario)/i;
...
// Generic progress-emit signals: a pulse/progress/event helper, an onProgress callback, or appending to a
// progress/event log. Broad on purpose so any "emit progress as you go" mechanism satisfies the gate.
const HAS_EVENTS = /(pulse|progress|emit[A-Z(]|onProgress|reportProgress|\bevents?\b|append[A-Za-z]*\((?:[^)]*)(?:log|progress|events)|console\.(?:log|info)\([^)]*\b(?:task|progress|done|\d+\/\d+))/i;
...
/** Which of the three required pattern markers are MISSING from a runner's source. */
export function missingMarkers(source) {
  ...
  if (!HAS_EVENTS.test(source)) missing.push('PROGRESS (emit plain-English progress as each task advances — a pulse / progress log / event, so a long run is observable live)');
...
The rule: EVERY benchmark must run its tasks IN PARALLEL, EMIT regular progress as it goes, and be
DURABLE + IDEMPOTENT.
...
Fix the runner to:
  ...
  2. Emit progress: a Goal, then a line per real step, then a pass/fail summary, to a progress
     log / pulse / event stream so the run is observable while it's happening.
